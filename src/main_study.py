...
def main_test(win, image_arr):
    ...
    for idx, image_name in enumerate(image_arr):
        ...
        image_ui = visual.ImageStim(win, image=image_name, pos=(0, IMAGE_BIAS_HEIGHT_1),
...
def main_study(win, stu_list, fw, n=1):
    ...
    for idx, stu_list in enumerate(stu_list_all):
        if idx != 0:
            ...
            core.wait(0.5)
        for stu in stu_list:
            query_intent = ['下一类别: ' + stu['category'], " 请按空格键继续"]
            ...
            text_ui.draw()
            win.flip()
            send_trigger(TRIGGER_LIST['read_category'])

            event.waitKeys(keyList=['space'])
            pair_info = main_test(win, stu['image'])
